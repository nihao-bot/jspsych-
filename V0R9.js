 ...
 {timeline:[
  {
   ...
   {stimulus:`<p style="color:red;font-weight:bold;font-size:120px;">红</p>`,choices:['f','j'],
    on_finish:function(data){
      let correct = jsPsych.timelineVariable('correct');
...
loop_function:function(){
  let practice_data = jsPsych.data.get().filter({trial_type: 'jsPsychHtmlKeyboardResponse'}).values();
    // 获取正确的试次数量
    let practice_data = jsPsych.data.get().filter({ trial_type: 'jsPsychHtmlKeyboardResponse' });
    // 获取正确的试次数量
    let correct_trials = practice_data.filter({ correct: true }).count();
    // 获取总的试次数量
    let total_trials = practice_data.count();
    // 如果正确的试次数量小于总的试次数量，返回 true，练习阶段将会重复

...
let practiceTrials = {
  timeline: [
      {stimulus: '+', choices: 'NO_KEYS', trial_duration: 300},
      {
          ...
          on_finish: function(data) {
              var correct = jsPsych.timelineVariable('correct', true);
              var response = data.response;
              var was_correct = (response === correct);
              jsPsych.data.addDataToLastTrial({correct: correct, response: response, was_correct: was_correct});
              var feedback = was_correct ? 
                  '<p style="color:green;">正确！</p>' : 
                  '<p style="color:red;">错误！</p><p style="color:red;">正确答案是 "' + correct + '"。</p>';
  ...
  timeline_variables: [
      ...
      {content: `<p style="color:blue;font-weight:bold;font-size:120px;">白</p>`, correct: 'j'},
      {content: `<p style="color:blue;font-weight:bold;font-size:120px;">红</p>`, correct: 'j'},
      {content: `<p style="color:red;font-weight:bold;font-size:120px;">白</p>`, correct: 'f'}
  ],
  randomize_order: true,
  on_finish: function() {
      checkAccuracyAndProceed();
  }
};

// 检查练习阶段的准确性，并决定是否进入正式试验阶段
function checkAccuracyAndProceed() {
  let practiceData = jsPsych.data.get().filter({trial_type: 'html-keyboard-response'}).values();
  let correctCount = practiceData.filter(function(trial) {
      return trial.was_correct === true;
  }).length;
  let totalTrials = practiceData.length;
  let accuracy = (correctCount / totalTrials) * 100;

  if (accuracy < 100) {
      // 如果正确率不足100%，则重新初始化练习阶段
      jsPsych.init({
          timeline: [practiceTrials],
          on_finish: checkAccuracyAndProceed 
      });
  } else {
      // 如果正确率达到100%，则进入试验阶段
      jsPsych.run([trials]);
...
// 启动实验，运行包含trial1和practiceTrials的试验序列。
jsPsych.run([trial1, practiceTrials]);
